    ...
    <main className="mx-auto min-h-screen max-w-lg px-6 py-10">
      ...
      <div className="flex flex-col gap-8">
        ...
        <section className="rounded-2xl border border-border bg-panel p-6 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-text">City <span className="text-muted text-xs">(helps find nearby players)</span></label>
            <input
        ...
        <section className="rounded-2xl border border-border bg-panel p-6 flex flex-col gap-5">
          ...
          <div className="flex flex-wrap gap-3">
            {FALLBACK_GAMES.filter((g) => !profile.games.includes(g)).map((g) => (
              <Chip key={g} label={g} selected={false} onClick={() => setProfile({ games: [...profile.games, g] })} />
            ))}
          </div>
            ...
            {showSuggestions && dropdownSuggestions.length > 0 && (
              <div className="absolute left-0 right-0 top-full z-10 mt-1 rounded-xl border border-border bg-panel shadow-lg overflow-hidden">
                {dropdownSuggestions.map((g) => (
                  <button key={g} type="button" onMouseDown={(e) => { e.preventDefault(); addGame(g); }} className="flex w-full items-center px-4 py-3 text-sm text-text hover:bg-panel2 transition">{g}</button>
                ))}
              </div>
            )}
          </div>
        ...
        <section className="rounded-2xl border border-border bg-panel p-6 flex flex-col gap-5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-text mb-3">Platform <span className="text-accent">*</span> <span className="text-muted font-normal">(select all that apply)</span></h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-text mb-3">Play style <span className="text-accent">*</span> <span className="text-muted font-normal">(select all that apply)</span></h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-text mb-3">Language <span className="text-accent">*</span></h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-text mb-3">Availability <span className="text-muted font-normal">(optional)</span></h3>
        ...
        <div className="flex flex-col gap-3 pb-10">
          <button onClick={handleSave} disabled={loading} className="w-full rounded-xl bg-accent px-6 py-3 text-sm font-semibold text-black hover:opacity-90 disabled:opacity-50 transition">
            {loading ? 'Saving…' : 'Save changes ✓'}
          </button>
          <Link href="/matches" className="w-full rounded-xl border border-border px-6 py-3 text-sm font-semibold text-text text-center hover:bg-panel2 transition">Cancel</Link>
        </div>
